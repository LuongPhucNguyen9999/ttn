...
class Validate
{
	...
	// Validate String
	private function validateString($element, $min = 0, $max = 30)
	{
	...
	private function validateStatus($element, $options)
	{
		if (in_array($this->source[$element], $options['deny']) == true) {
			$this->setError($element, 'Please choose another value differ with default value !');
		}
	}
	...
	private function validateNotExistRecord($element, $options)
	{
		...
		if ($database->isExist($query) == true) {
			$this->setError($element, 'this username has exist');
		}
	}
